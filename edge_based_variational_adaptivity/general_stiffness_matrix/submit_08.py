...
python experiment_08.py --theta {theta} --fudge {fudge} --batchsize {batchsize} --miniter {miniter}
"""
...
for i, (theta, fudge, batchsize, miniter) in enumerate(combinations):
    ...
    job_name = f"exp-08_theta-{theta}_fudge-{fudge}_batchsize-{batchsize}_miniter-{miniter}"
