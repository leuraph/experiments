...
python experiment_10.py --theta {theta} --fudge {fudge} --batchsize {batchsize} --miniter {miniter}
"""
...
for i, (theta, fudge, batchsize, miniter) in enumerate(combinations):
    ...
    job_name = f"exp-10_theta-{theta}_fudge-{fudge}_batchsize-{batchsize}_miniter-{miniter}"
