...
python experiment_09.py --theta {theta} --fudge {fudge} --initial_delay {delay} --miniter {miniter} --tau {tau} --delay_increase {delay_increase}
"""
...
for i, (theta, fudge, delay, miniter, delay_increase, tau) in enumerate(combinations):
    ...
    job_name = f"exp-09_theta-{theta}_fudge-{fudge}_delay-{delay}_miniter-{miniter}_delay_increase-{delay_increase}_tau-{tau}"
